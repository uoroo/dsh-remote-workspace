...
解决 **DeepSeek Harness 远程访问操作不便** 的问题：harness WebUI 默认只监听 `127.0.0.1:3080`（不能绑定 0.0.0.0）、页面没有登录认证，而且「添加工作区 → 选择文件夹」会直接在**宿主机桌面**弹出系统文件夹选择窗口——远程访问时没有远程桌面就卡死在下一步。本仓库补齐这条链路，并新增**挂载非 harness 宿主机文件夹**（SSHFS）的能力。

具体解决的痛点：

| 痛点 | 本仓库方案 |
|------|-----------|
| harness 不能绑定 `0.0.0.0`，只能本机访问 | `dsh-proxy`（aiohttp 反向代理）监听 `0.0.0.0:6677`，转发到 `127.0.0.1:3080` |
| 网页没有登录认证，暴露即裸奔 | 密码登录表单 + HMAC 无状态 cookie（7 天免登），未登录一律返回登录页 |
| 网页上没法选择工作区（原生 harness 会在**宿主机桌面**弹系统文件夹选择窗口，需远程桌面才能继续操作） | 「添加工作区」改为网页内三选一：原生弹窗 / Web 文件管理器 / SSHFS 挂载，全程浏览器内完成 |
| 想挂载**其他机器**（非 harness 宿主机）的文件夹作为工作区 | SSHFS 表单：已保存 SSH 配置下拉自动填充 + sftp 远程目录浏览 + 一键挂载并采纳为工作区 |
...
> 图为局域网远程访问 `http://192.168.1.8:6677` 的实际效果。
